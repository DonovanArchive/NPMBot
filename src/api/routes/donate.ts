		...
		app
			...
			.post("/done/ko-fi", async (req, res) => {
				...
				const e = new EmbedBuilder(config.devLanguage)
					...
					.setAuthor(pub ? !u ? b.from_name : `${u.username}#${u.discriminator}` : "{lang:other.words.private$ucwords$}", pub && u ? u.avatarURL : undefined)
					.setDescription(pub ? b.message : "[{lang:other.words.message$ucwords$} {lang:other.words.hidden$ucwords$}]")
					.setFooter(`{lang:other.words.amount$ucwords$}: ${[null, "USD"].includes(b.currency) ? `$${b.amount}` : `${b.amount} ${b.currency}`} | {lang:other.donations.ko-fi.user}: ${pub ? b.from_name : "{lang:other.words.private$ucwords$}"}`, client.user.avatarURL);
				...
				if (user && u) {
					...
					d = await u.getDMChannel().then(dm => dm.createMessage({
						embed: new EmbedBuilder(config.devLanguage)
							...
							.setFooter(`{lang:other.donations.ko-fi.dmFooter|${config.emojis.default.blueHeart}}`, client.user.avatarURL)
							.setTimestamp(new Date().toISOString())
							.setColor(Colors.gold)
