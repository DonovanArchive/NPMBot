...
export default class DeleteRoute extends Route {
	...
	setup() {
		...
		app
			...
			.get("/user", async (req, res) => res.redirect(`/delete/user/${req.data.user!.id}`))
			.get("/user/:id", async (req, res) => {
				const u = await db.getUser(req.data.user!.id);
				if (req.params.id !== req.data.user!.id && !config.developers.includes(req.data.user!.id)) return res.status(404).render("error", { title: "You cannot delete others data." });
				if (!u) return res.status(404).render("error", { title: "Not Found", status: 404, message: `We couldn't find any data stored for you. Make sure you are signed into the right account (${req.data.user!.username}#${req.data.user!.discriminator}). You can <a href="/discord/logout">logout</a> here.` });
				if (u.deletion) {
					if (config.developers.includes(req.data.user!.id)) return res.status(400).render("error", { title: "Process Already Started", status: 400, message: "That user is already marked for deletion." });
			...
			.get("/user/:id/confirm", async (req, res) => {
				...
				const u = await db.getUser(req.data.user!.id);
				if (req.params.id !== req.data.user!.id && !config.developers.includes(req.data.user!.id)) return res.status(404).render("error", { title: "You cannot delete others data." });
				if (!u) return res.status(404).render("error", { title: "No Data Found", status: 400, message: `We couldn't find any data stored for you. Make sure you are signed into the right account (${req.data.user!.username}#${req.data.user!.discriminator}). You can <a href="/discord/logout">logout</a> here.` });
				if (u.deletion) {
					if (config.developers.includes(req.data.user!.id)) return res.status(400).render("error", { title: "Process Already Started", status: 400, message: "That user is already marked for deletion." });
					else return res.status(400).render("error", { title: "Process Already Started", status: 400, message: "You have already started the deletion process." });
				}
			...
			.get("/guild/:id", async (req, res) => {
				...
				if (!config.developers.includes(req.data.user!.id)) {
					const j = await client.getGuild(req.params.id);
					...
					const m = await j.getRESTMember(req.data.user!.id);
					...
					if (j.ownerID !== req.data.user!.id) return res.status(404).render("error", { title: "Unauthorized", status: 401, message: `Only the server owner (${o!.username}#${o!.discriminator}) can perform this action.` });
				}
			...
			.get("/guild/:id/confirm", async (req, res) => {
				...
				if (!config.developers.includes(req.data.user!.id)) {
					const j = await client.getGuild(req.params.id);
					if (!j) return res.status(404).render("error", { title: "Forbidden", status: 403, message: "Due to me being removed from that server, I cannot verify your ability to perform this action. Please contact a staff member to further this process." });
					const m = j.getRESTMember(req.data.user!.id).catch(err => null);
					if (!m) return res.status(404).render("error", { title: "Unauthorized", status: 401, message: "You are not in this server, so you cannot perform this action." });
					const o = j.members.get(j.ownerID);
					if (j.ownerID !== req.data.user!.id) return res.status(404).render("error", { title: "Unauthorized", status: 401, message: `Only the server owner (${o!.username}#${o!.discriminator}) can perform this action.` });
				}
