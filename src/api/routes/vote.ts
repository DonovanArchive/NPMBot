	...
	setup() {
		...
		/* app
			.post("/dbl", async (req: express.Request<unknown, unknown, Vote.DBLVote>, res) => {
				...
				const u = client.users.has(req.body.user) ? client.users.get(req.body.user) : await client.getUser(req.body.user);
			...
			.post("/dboats", async (req: express.Request<unknown, unknown, Vote.DBoatsVote>, res) => {
				...
				const u = client.users.has(req.body.user.id) ? client.users.get(req.body.user.id) : await client.getUser(req.body.user.id);
				await client.w.get("vote").execute({
					embeds: [
						{
							title: "Vote Performed",
