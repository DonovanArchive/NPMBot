...
export default class API {
	...
	launch() {
		...
		const svr: http.Server = http.createServer(express())
			.on("error", () => {
				Logger.warn(["API Server"], `[${this.launchAttempts}/${this.maxAttempts}]: Attempted to start api server, but it was not able to bind. Is the port in use? ${this.launchAttempts >= this.maxAttempts ? "Max start attempts reached, not attempting to start anymore." : `Attempting to start again in ${this.timeBetweenAttempts / 1000} seconds.`}`);
