...
export default class Utility {
	 ...
	 */
	static toStringFormat<T extends object>(d: T) {
	...
	static numberToEmoji(num: number | string) {
		...
		Object.keys2(m).map(v => num = num.toString().replace(new RegExp(v.toString(), "g"), m[v]));
		return num;
	}
	 ...
	 *
	 * @static
	 * @param {NPMBot} client - The bot client.
	...
	static async logError(client: NPMBot, err: Error, type: "event", extra: {}): Promise<{
		message: Eris.Message<Eris.TextableChannel>;
		code: string;
	}>;
	static async logError(client: NPMBot, err: Error, type: "message", extra: ExtendedMessage): Promise<{
		message: Eris.Message<Eris.TextableChannel>;
		code: string;
	}>;
	// eslint-disable-next-line @typescript-eslint/ban-types
	static async logError(client: NPMBot, err: Error, type: "message" | "event", extra?: ExtendedMessage | {}): Promise<{
		message: Eris.Message<Eris.TextableChannel>;
		code: string;
	}> {
		if ([
		...
		const p = await client.createPaste(err.stack!, "NPMBot Error", "1W", 2);
