...
export default class StatsHandler {
	...
	async getUserStats(id: string) {
		...
		const servers = this.#client.guilds.filter(g => g.members.has(id)).map(s => s.id);
		const channels: string[] = servers.map(s => Array.from(this.#client.guilds.get(s)!.channels || [null, null]).map(c => c[1])).reduce((a, b) => a.concat(b), []).filter(c => !!c).map(c => c.id);
		const stats = {
			messages: {
