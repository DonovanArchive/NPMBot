...
export default class Command {
	...
	run: (this: NPMBot, msg: ExtendedMessage, cmd: Command) => Promise<any>;
	// allow isn't used right now but it can be a bypass system in the future
	overrides:
		{
			permissionError: (this: NPMBot, msg: ExtendedMessage, cmd: Command, type: "user" | "bot", permissions: ErisPermissions[]) => Promise<OverrideReturn>;
			invalidUsage: (this: NPMBot, msg: ExtendedMessage, cmd: Command, err: CommandError<"ERR_INVALID_USAGE">) => Promise<OverrideReturn>;
			help: (this: NPMBot, msg: ExtendedMessage, cmd: Command) => Promise<OverrideReturn>;
			cooldown: (this: NPMBot, msg: ExtendedMessage, cmd: Command, time: number) => Promise<OverrideReturn>;
		} & {
			[k in "beta" | "developer" | "donator" | "guildOwner" | "nsfw" | "premium" | "supportServer"]: (this: NPMBot, msg: ExtendedMessage, cmd: Command) => Promise<OverrideReturn>;
		};
	file: string;
	...
	runOverride<K extends keyof Command["overrides"]>(type: K, client: NPMBot, ...args: Parameters<Command["overrides"][K]>): ReturnType<Command["overrides"][K]> {
