...
export default new Command(["eval", "ev"], __filename)
	...
	.setExecutor(async function (msg, cmd) {
		...
		if (!silent) {
			if (res.length > 1000) {
				const pasteURL = await Request.createPaste(res, "NPMBot Eval", "1H", 2);
				res = `Uploaded ${pasteURL}`;
			}

			if (error) {
				Logger.error(`Shard #${msg.channel.guild.shard.id}`, o ?? res);
			...
			return msg.channel.createMessage({
				embed: new EmbedBuilder(config.defaults.config.guild.settings.lang as any)
					...
					.addField(":outbox_tray: Output", `\`\`\`js\n${res}\`\`\`${error && res.indexOf("Uploaded") === -1 ? `\n[stack (hover)](https://furry.bot '${(stack as any).replace(/'/g, "\u0060")}')` : ""}`, false)
					.toJSON()
			});
		} else {
			if (res.length > 3000) {
				const pasteURL = await Request.createPaste(res, "NPMBot Silent Eval", "1H", 2);
				res = pasteURL;
			}
