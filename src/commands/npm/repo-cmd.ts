...
export default new Command(["repo"], __filename)
	.setBotPermissions([])
	.setUserPermissions([])
	.setRestrictions([])
	...
	.setExecutor(async function (msg, cmd) {
		...
		return msg.reply({
			embed: new EmbedBuilder(msg.gConfig.settings.lang)
				.setTitle(`${pkg.name}@${ver.version} - repo`)
				.setDescription(!ver.repository?.url ? `{lang:${cmd.lang}.none|${p.name}}` : `[${ver.repository.url.replace(/(\.git|git\+)/g, "")}](${ver.repository.url.replace(/(\.git|git\+)/g, "")})`)
				.setColor(Colors.npm)
